...
fun game() {            // Игра()
    ...
    do {
        if (isPat(player)) {                                                       // Пат?() - проверяем перед ходом, чтобы сменился игрок
            println("Stalemate!")                                                  // и можно было пользоваться всеми нужными методами, а не писать новые
            break                                                                   
        }
...
fun move(player: String): Boolean {                                                 // Ход()
    ...
    when {                                                                            // Когда
        // 1. Неверный ход
        start != myColor -> println("No $color pawn at ${enter.substring(0, 2)}")     // Нет своей пешки для хода в начальной клетке
        // 2. Неверный ход
        ...
        normalMove() -> {                                                              // обычныйХод()
            ...
            isWins()                                                                   // победа?()
            return true                                                                // возврат тру, чтобы остаться в цикле вайл и сменить игрока
        }                                                                               
    ...
    return false                                                                        // возврат false, чтобы остаться в цикле, не меняя игрока, т.к. ход был неверный
}                                                                                       

fun normalMove() =                                                                      // обычныйХод()
    (start == myColor && end != oppColor &&                                             // Есть своя пешка на начальной клетке и нет пешки противника на конечной клетке и                                                                                         
     board[step[DIGIT_START ] - vector][step[LITER_START]] != oppColor &&               // нет пешки противника на клетке перед, если ход через клетку, чтобы не перескочить                                                                                         
     step[LITER_END] == step[LITER_START])                                              // и ход идет по прямой

fun capturingTheOpponentPawn() =                                                        // взятиеПешкиПротивника()
    start == myColor && end == oppColor &&                                              // На стартовой клетке есть своя и на конечной есть противник                                                                                         
...
fun capturingAnOpponentPassedPawn() =                                                   // взятиеПроходнойПешкиПротивника()
    (start == myColor && end != oppColor && isOppPawnNearby()  &&                         // На стартовой клетке есть своя и на конечной есть противник и пешка соперника рядом и                                                                                         
...
fun isNormalMove(i: Int, j: Int, player: String): Boolean {                                 // нормальныйХод?() - проверяем простой ход вперед           
    vector = if (player == firstPlayer) 1 else -1                                           // Определяем переменные. т.к. они тоже от старого игрока еще 
    step = arrayOf(i, j, i - vector, j)                                                     // Изменяем конечную клетку на 1 вручную с помощью vector
    start = board[step[DIGIT_START]][step[LITER_START]]                                     // для каждого игрока будет свой знак направления        
...
fun isCapturingTheOpponentPawn(i: Int, j: Int, player: String): Boolean {                    // возможноВзятиеПешкиПротивника?()
    ...
    var patPawn = 0
    if (j != 7) {                                                                            // Проверяем отдельно в двух условиях крайние клетки, чтобы не выйти за пределы массива
...
fun isOppPawnNearby ():Boolean {                                                            // пешкаСоперникаПоблизости?()
